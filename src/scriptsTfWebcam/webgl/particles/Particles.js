...
export default class Particles {
  ...
  init(src) {
    ...
    let canvasVideo1 = document.getElementById("canvasVideo");

    
    removeBackground(video1, newCanvas);
 
    
    function canvasToVideo(canvas, fps) {
      // Create a new Whammy.Video object with the specified frame rate
      const video = new Whammy.Video(fps);
            
    video.add(canvas);
      
    
      // Create a new video element and set the source to the Blob URL for the video
      const videoElement = document.createElement('video');
      videoElement.src = URL.createObjectURL(video.compile());
    
      // Return the video element
      return videoElement;
    }

    const canvasVideo = canvasToVideo(newCanvas , 60);

 //  updateVideo(canvasVideo);
    ...
    canvasVideo.addEventListener("canplay", () => {
      ...
      this.width = canvasVideo.videoWidth;
      this.height = canvasVideo.videoHeight;
